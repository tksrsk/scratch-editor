...
class Scratch3MicroBitBlocks {
    ...
    constructor (runtime) {
        ...
        this._maximumSpeed = 150;
        this._maximumSpeedConfigured = false;
        this._timeToMaximumSpeed = 15;
    ...
    getInfo () {
        return {
            ...
            blocks: [
                ...
                '---',
                {
                    opcode: 'setMaximumSpeed',
                    text: '最高速度 [MAX_SPEED] km/hまで [SECONDS] 秒で加速する',
                    blockType: BlockType.COMMAND,
                    arguments: {
                        MAX_SPEED: {
    ...
    setMaximumSpeed (args) {
        this._maximumSpeed = Math.max(1, cast.toNumber(args.MAX_SPEED));
        this._timeToMaximumSpeed = Math.max(0.1, cast.toNumber(args.SECONDS));
        this._maximumSpeedConfigured = true;
        this._acceleration = 0;
    }
    ...
    isReady () {
        return this._maximumSpeedConfigured &&
            this._releasedAngle !== null &&
            this._pressedAngle !== null &&
    ...
    /**
     * Get the current pedal angle, using the up arrow as a virtual accelerator
    ...
    _getPedalAngle () {
        ...
        if (!keyboard || typeof keyboard.getKeyIsDown !== 'function' ||
            !keyboard.getKeyIsDown('up arrow')) return released;
        return this._pressedAngle === null ? released - 30 : this._pressedAngle;
    }

    updateAcceleration (args) {
        if (!this.isReady()) {
            this._acceleration = 0;
            return;
        }
        ...
        const targetSpeed = pedal * this._maximumSpeed;
        if (targetSpeed > 0 && speed < targetSpeed) {
            const speedRatio = Math.max(0, Math.min(1, speed / targetSpeed));
            const normalizedTime = Math.acos(1 - (2 * speedRatio)) / Math.PI;
            const nextNormalizedTime = Math.min(1, normalizedTime + (0.1 / this._timeToMaximumSpeed));
            const nextSpeedRatio = (1 - Math.cos(Math.PI * nextNormalizedTime)) / 2;
            const nextSpeed = targetSpeed * nextSpeedRatio;
            this._acceleration = Math.min(
                (nextSpeed - speed) / 0.1,
    ...
    getAcceleration () {
        return this.isReady() ? this._acceleration : 0;
    }
