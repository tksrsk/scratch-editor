...
class Scratch3MicroBitBlocks {
    ...
    constructor (runtime) {
        ...
        this._centerAngle = null;
        this._rightAngle = null;
        this._leftAngle = null;
    }
    ...
    getInfo () {
        return {
            ...
            blocks: [
                ...
                '---',
                {
                    opcode: 'recordCenterAngle',
                    text: 'まっすぐの位置を記録する',
                    blockType: BlockType.COMMAND
                },
                {
                    opcode: 'waitForCenterAngle',
                    text: 'まっすぐの位置が安定するまで待つ',
                    blockType: BlockType.COMMAND
                },
                {
                    opcode: 'recordRightAngle',
                    text: '右いっぱいの位置を記録する',
                    blockType: BlockType.COMMAND
                },
                {
                    opcode: 'waitForRightAngle',
                    text: '右に回した位置が安定するまで待つ',
                    blockType: BlockType.COMMAND
                },
                {
                    opcode: 'recordLeftAngle',
                    text: '左いっぱいの位置を記録する',
                    blockType: BlockType.COMMAND
                },
                {
                    opcode: 'waitForLeftAngle',
                    text: '左に回した位置が安定するまで待つ',
                    blockType: BlockType.COMMAND
                },
                {
                    opcode: 'isReady',
    ...
    waitForLeftAngle (args, util) {
        ...
        const oppositeSide = Math.abs(leftRange) >= 5 && rightRange * leftRange < 0;
        if (this._waitForStableAngle(angle, util, oppositeSide)) this._leftAngle = angle;
    }

    _waitForStableAngle (angle, util, mayComplete = true) {
        ...
        if (Date.now() - frame.stableSince >= 500) return true;
        util.yield();
        return false;
    }

    isReady () {
        if (this._centerAngle === null || this._rightAngle === null || this._leftAngle === null) return false;
        const rightRange = this._rightAngle - this._centerAngle;
        const leftRange = this._leftAngle - this._centerAngle;
        return Math.abs(rightRange) >= 5 &&
            Math.abs(leftRange) >= 5 &&
    ...
    getSteeringAmount () {
        ...
        const rightRange = this._rightAngle - this._centerAngle;
        const leftRange = this._leftAngle - this._centerAngle;
        const amount = offset * rightRange >= 0 ?
